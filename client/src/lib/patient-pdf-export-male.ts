...
export async function generateMalePatientWellnessPDF(
  ...
  if (!sectionHidden('maleHormonePatterns') && interpretation.maleHormonePatterns && interpretation.maleHormonePatterns.length > 0) {
    ...
    const patientPatternDescriptions: Record<string, string> = {
      'Primary Testicular Failure Pattern': 'Your brain is sending strong signals to produce testosterone (shown by elevated LH), but testosterone output is still lower than ideal. This pattern suggests a limitation within the testes themselves. Your provider will discuss next steps, which may include testosterone optimization therapy.',
      'Secondary / Central Suppression Pattern': 'Your testosterone is below the optimal range, and the brain\'s hormonal signal (LH) is also lower than expected. This pattern suggests the brain\'s hormone control center may not be sending a strong enough signal. Your provider will evaluate underlying causes and discuss treatment options.',
      'Obesity / Low SHBG Functional Low-T Pattern': 'Your total testosterone reading may appear lower than it really is because a protein called SHBG is reduced — often linked to excess body weight or insulin resistance. What matters most is your free (biologically active) testosterone. Your provider will review your full metabolic picture and guide next steps.',
      'High SHBG / Low Free Testosterone Pattern': 'You have enough total testosterone, but a protein called SHBG may be "locking up" much of it, leaving less available for your body to actually use. This can cause symptoms of low testosterone — such as fatigue, low libido, or reduced muscle — even when your total level appears normal on paper. Targeted lifestyle changes and therapies may improve how you feel.',
      'Aromatization / Estrogen-Dominant Pattern': 'Your estradiol (a form of estrogen that men naturally carry in small amounts) is higher than ideal relative to your testosterone. In men, estradiol needs to stay in balance — imbalances can affect energy, mood, libido, and body composition. Your provider will discuss the best approach to restore balance.',
      'Over-Replacement / Supraphysiologic TRT Pattern': 'Your testosterone is above the physiologic range, which sometimes happens with testosterone therapy. Very high levels can affect red blood cell count, cardiovascular health, and other markers. Your provider will review your dose and make any needed adjustments to bring levels into a safer, optimal range.',
      'TRT Non-Responder / Symptom-Mismatch Pattern': 'Your testosterone levels are in an optimized range, but you may still have symptoms. This pattern suggests your symptoms may be driven by something other than testosterone — such as sleep quality, thyroid function, iron levels, or metabolic health. Your provider will investigate further.',
      'Fertility-Preservation Pattern': 'Your testosterone is in a range where starting traditional testosterone therapy could reduce sperm production. If preserving fertility is important to you, your provider will discuss alternative approaches — such as clomiphene or hCG — that can support hormone levels without suppressing sperm production.',
    };
