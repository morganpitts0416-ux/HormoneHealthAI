...
program aha_prevent_uacr, rclass
...
syntax [if], age(varname numeric) sex(varname numeric) bmi(varname numeric) sbp(varname numeric) bptreat(varname numeric) tc(varname numeric) hdl(varname numeric) statin(varname numeric) dm(varname numeric) smoke(varname numeric) egfr(varname numeric) uacr(varname numeric)

marksample touse, novarlist
markout `touse' `age' `sex' `bmi' `sbp' `bptreat' `tc' `hdl' `statin' `dm' `smoke' `egfr' `uacr', sysmissok
...
if "`o'"=="hf" local cov `prevent_sbp_1' `prevent_sbp_2' `dm' `smoke' `prevent_bmi_1' `prevent_bmi_2' `prevent_egfr_1' `prevent_egfr_2' `bptreat' `prevent_bptreat' `age_`prevent_sbp_2'' `age_`dm'' `age_`smoke'' `age_`prevent_bmi_2'' `age_`prevent_egfr_1''
	...
	if `y'==30 local av `prevent_age' `prevent_age2'
		...
		local var2 `prevent_logacr' `prevent_acr_missing'
		forvalues i=2/2 {
		...
		qui gen aha_prevent_uacr_`y'yr_`o'=exp(`xb`i'_`o'_`y'')/(exp(`xb`i'_`o'_`y'')+1) if `touse' & `sex'!=.
		}
	}
}

foreach v of varlist aha_prevent_uacr_* {
	qui replace `v'=`v'*100
}
...
label variable aha_prevent_uacr_10yr_cvd "AHA PREVENT Risk UACR Model: Any CVD in 10 years (%)"
label variable aha_prevent_uacr_30yr_cvd "AHA PREVENT Risk UACR Model: Any CVD in 30 years (%)"
label variable aha_prevent_uacr_10yr_ascvd "AHA PREVENT Risk UACR Model: ASCVD in 10 years (%)"
label variable aha_prevent_uacr_30yr_ascvd "AHA PREVENT Risk UACR Model: ASCVD in 30 years (%)"
label variable aha_prevent_uacr_10yr_hf "AHA PREVENT Risk UACR Model: Heart failure in 10 years (%)"
label variable aha_prevent_uacr_30yr_hf "AHA PREVENT Risk UACR Model: Heart failure in 30 years (%)"
