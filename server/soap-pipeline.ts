...
The note documents the clinical meaning of the encounter in natural provider language — not a recap of the conversation. Read and reason over the full transcript; document the clinical picture.
...
Use medical terminology where it adds clinical precision, but do not replace ordinary clinical language with technical terminology simply to sound formal. Adding clinical precision means increasing specificity or reducing ambiguity — it does not mean replacing a patient's plain functional description with an abstract equivalent. "Food noise/appetite has returned" is already clinically clear; "appetite-related signaling has returned" is more abstract, not more precise.
...
40. SILENTLY RESOLVED AMBIGUITY — CLINICALLY CONSEQUENTIAL DECISION-STATE ONLY: Identify clinically consequential plan items where the transcript is genuinely ambiguous or contradictory — specifically, a treatment, medication, procedure, diagnostic test, referral, follow-up decision, or other clinically consequential plan item that appears both deferred and possibly approved within the same visit (e.g., "we were going to hold off... I guess we could add it in... you can get it in"). The note must NOT confidently state one outcome. Flag as CRITICAL, document the decision-state ambiguity neutrally in the note, and add the item to needs_clinician_review with both readings so the provider can confirm the actual decision. This check governs clinical decision-state ambiguity only. It does NOT apply to patient-reported symptoms, conversational references, or incidental transcript statements whose precise meaning is unclear but which do not affect a treatment plan, diagnosis, medication reconciliation, or safety decision. Uncertainty that does not affect a clinical decision must not be narrated in the note body.
