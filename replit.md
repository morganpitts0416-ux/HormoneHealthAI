...
-   **Patient Profiles Page** (`/patients`): Dedicated patient profile page with clinical snapshot (comparing latest vs prior labs with categorized improvements/concerns/urgent items/stable markers), clickable lab history with detail modals, enriched trend analysis, and visual trend charts. Accessible via "Patient Profiles" button from both lab interpretation pages.
