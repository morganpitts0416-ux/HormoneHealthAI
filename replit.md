...
-   **AI-Generated SOAP Notes**: Both clinics generate chart-ready SOAP notes (Subjective, Objective, Assessment, Plan) using AI, incorporating all lab values, red flags, risk scores, IR screening, and supplement recommendations. Displayed in results with copy-to-clipboard functionality and included in the provider PDF report. Located in `server/ai-service.ts` (`generateSOAPNote` method), displayed via `client/src/components/soap-note.tsx`.
