...
-   **Patient Profile Layout (compact tiles)**: Patient Chart panel sits at the TOP of every patient profile as the primary chart context. Below it, a single 3-column row of compact "status tiles" replaces the previously stacked full-width panels for **Appointments**, **Vitals Monitoring**, and **Check-Ins**. Each tile shows live summary data (counts, next-appointment date, latest BP reading, tracking mode, alert/unreviewed-med badges) and click-opens a Dialog containing the full detailed panel (mirrors the PREVENT Calc dialog pattern). The default-off contract for Daily Check-In is preserved: when a patient hasn't opted in, the Check-Ins dialog shows a friendly "not enrolled" explainer instead of an empty body. Tile summary queries reuse identical react-query keys as the inner panels, so no double-fetch occurs.
