...
-   **Clinical Block Defaults (Per-Clinician)**: Each clinician can customize the systems and per-system normal-finding text used when inserting a Review of Systems or Physical Examination block. Saved overrides persist per (clinic, provider) in the `clinical_block_defaults` table and flow through the slash menu, manual SOAP builder, and template builder. When a chart row is at status="normal" with no notes AND a defaultFinding override exists, the rendered note replaces "Normal/Negative" with the clinician's preferred wording (e.g. "RRR, no murmurs"). Settings UI lives at Account → Clinical Block Defaults; saving a value identical to shipped defaults stores null (treated as "use shipped").
...
-   **Medication & Supplement Requests Widget**: Clinician dashboard combines pending supplement orders and patient-portal medication refill requests in a single inbox-style widget (formerly "Supplement Orders"). Each row is tagged with a Pill or ShoppingBag icon and a kind label, sorted newest-first. Refill rows have a "Mark handled" button that dismisses the underlying provider_inbox_notification (type=med_refill_request); supplement order rows keep the existing "Mark fulfilled" action.
...
-   **Chart Review — Collaborating-Physician Oversight (Slice 2)**: Mid-level clinicians (NP/PA) configure a collaborating-physician agreement with quota tracking. (1) Quota kind selector toggles between percent of signed notes and a fixed monthly chart count. (2) Collaborating physician picker is restricted to MD/DO titles only (server-authoritative; client filters for UX). (3) Prospective full-gate workflow: when a Prospective agreement applies AND a mandatory rule fires (controlled substance, new diagnosis), the mid-level's "Sign" parks the chart in a `pending_collab_review` state — `lockedAt` is set, edits/amendments return HTTP 423, `signedAt` stays null, `signedBy` is stashed. The chart is officially signed only when the physician concurs; rejection clears the lock so the mid-level can edit and re-submit. Park + queue insert are atomic in a single DB transaction. (4) The voluntary "Send for review?" prompt in the sign dialog is suppressed once the monthly quota target is met (still shown for mandatory rules and prospective gating).
-   **Patient Documents Tab**: The "Documents" tab on a patient profile is composed of three collapsible cards: (1) **Supplement Orders** (pending/fulfilled supplement orders for the patient), (2) **Forms & Consents** (digital intake forms — pending assignments with portal/email/in-clinic delivery + completed submissions with review/sync status), (3) **Uploaded Documents** — clinician-only file storage for outside medical records, PA forms, PMP reports, imaging, external labs, referrals, consents, insurance cards, photo IDs, and other. Supports drag-and-drop multi-file upload (PDF + images), native camera capture ("Take Photo"), and a multi-page document scanner ("Scan") that snaps pages via the device camera and bundles them into a single PDF via jsPDF. Each document is tagged with a category and optional notes, lists with View (inline) / Download (attachment) / Delete actions. Files persist as base64 TEXT in the `patient_documents` table, scoped by `clinic_id` + `patient_id`. Server enforces 25 MB per file, 50 MB aggregate per request, 10 files per request, with proper 413/415 mappings; client mirrors the same limits with friendly preflight errors.
