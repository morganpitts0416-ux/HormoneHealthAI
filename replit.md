...
-   **Smart Intake + Digital Forms Module**: Clinicians can build, publish, and manage patient intake forms with 16 field types, smart field auto-linking to patient profiles, and patient portal access. Supports form reassignment and patient profile merging. Auto-matches form submissions to existing patient profiles by name+DOB or email, or creates new profiles automatically. Includes a "Sync to Profile" review dialog on form submission previews that lets clinicians selectively approve which clinical data (medications, allergies, medical/surgical/family/social history) gets merged into the patient chart, with duplicate detection — mirroring the SOAP note extraction approval workflow. Endpoints: `GET /api/form-submissions/:id/sync-preview`, `POST /api/form-submissions/:id/sync` (accepts `approvedItems` for selective sync). UI: `client/src/components/form-submission-preview.tsx` (`SyncReviewDialog`).
