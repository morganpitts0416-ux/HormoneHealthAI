...
-   **Signed & Locked Chart Notes (EMR)**: Electronic signing and locking of SOAP notes for immutability and audit trails, with amendment support. Both AI-generated and manual SOAP notes follow the same sign/amend workflow. Sign and Amend buttons are available directly in the Patient Profiles encounter list. The `/sign` endpoint detects amendments via `encounterVersions` audit trail (not just `signedAt`) so amend→re-sign correctly sets `isAmended`.
-   **Manual SOAP Notes**: Block-based note builder (`client/src/components/manual-soap-builder.tsx`) with 12 section types (HPI, ROS, Physical Exam, Assessment/Plan, etc.). Supports chart-mode editors for ROS/PE, structured Assessment/Plan with ICD-10 search. `/dx` shortcode in any textarea triggers inline diagnosis search (`client/src/components/diagnosis-search.tsx`). Block insertion uses `onMouseDown` handlers for reliable dropdown behavior within dialogs.
-   **Smart Intake + Digital Forms Module**: Clinicians can build, publish, and manage patient intake forms with 20 field types, smart field auto-linking to patient profiles, and patient portal access. Supports form reassignment and patient profile merging. Auto-matches form submissions to existing patient profiles by name+DOB or email, or creates new profiles automatically. Includes a "Sync to Profile" review dialog on form submission previews that lets clinicians selectively approve which clinical data (medications, allergies, medical/surgical/family/social history) gets merged into the patient chart, with duplicate detection — mirroring the SOAP note extraction approval workflow. **Clinical list field types** (`medication_list`, `allergy_list`, `medical_history_list`, `surgical_history_list`): One-item-at-a-time entry with "+" button, each item stored individually as an array element. **Family History Chart** (`family_history_chart`): Structured chart with 8 family members (Mother, Father, Maternal/Paternal Grandparents, Siblings, Children) listed on the left with text fields for conditions on the right; syncs as "Member: conditions" strings. **Signature field**: Draw (canvas pad) or Type (full legal name rendered in cursive) tabs, both saved as data URL for official medical documents. Endpoints: `GET /api/form-submissions/:id/sync-preview`, `POST /api/form-submissions/:id/sync` (accepts `approvedItems` for selective sync). UI: `client/src/components/form-submission-preview.tsx` (`SyncReviewDialog`).
-   **Daily Check-In (Phase 1) — Patient Tracking**: Optional, opt-in patient tracking with strict default-off behavior. Patients can enable Standard Mode tracking from `/portal/dashboard` (`DailyCheckInCard` opt-in card) or `/portal/check-in` (full page with left icon rail: Food, Sleep, Movement, Mood, Symptoms, Cycle [female only], Meds). Five new tables (`patient_tracking_settings`, `patient_daily_checkins`, `patient_medication_adherence_logs`, `patient_reported_medications`, `provider_inbox_notifications`) added in `shared/schema.ts` with idempotent `CREATE TABLE IF NOT EXISTS` in `server/prod-migrate.sql`. Patient endpoints under `/api/portal/tracking/*` (settings, daily check-ins, medication adherence, patient-reported medications); clinician endpoints `/api/patients/:id/tracking-summary` (uses scoped `storage.getPatient(id, clinicianId, clinicId)`) and `/api/clinician/inbox-notifications/*` (list/mark-read/mark-all-read/delete). Server auto-creates urgent inbox notifications for `unexpected_bleeding_reported` (from check-ins) and normal-severity `patient_added_med_or_supplement` notifications when patients add their own medications/supplements. AppHeader's bell badge sums unread message count + unread inbox notification count. Inbox page (`client/src/pages/inbox.tsx`) renders a "Notifications" section above "Patient Messages" with delete/mark-read controls. `MonitoringPanel` component at the bottom of `client/src/pages/patient-profiles.tsx` renders **only** when patient `trackingMode !== "off"` — non-enrolled patients show zero UI change. `markPatientReportedMedReviewed(id, patientId, reviewedByUserId)` storage method enforces patientId in the WHERE clause so cross-patient/cross-tenant mutations are impossible.
