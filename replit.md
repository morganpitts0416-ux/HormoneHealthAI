...
-   **Patient Profile Layout (compact tiles)**: Patient Chart panel sits at the TOP of every patient profile as the primary chart context. Below it, a 3-column row of compact "status tiles" for **Appointments**, **Portal & Messages**, and **Check-Ins**. Each tile shows live summary data (counts, next-appointment date, portal status, last-login date, unread-message badge, tracking mode, unreviewed-med badges). The Appointments and Check-Ins tiles open a Dialog containing the full detailed panel; the Portal & Messages tile (chevron rotates open/closed) toggles an inline panel below the row that shows portal account/login/report status, with a "Show messages" button that auto-expands when first opened so reading new patient messages stays one-click. The Vitals area is accessed exclusively via the action-bar **Vitals** button (see Unified Vitals Dialog below) — there is no separate Vitals Monitoring tile. The default-off contract for Daily Check-In is preserved: when a patient hasn't opted in, the Check-Ins dialog shows a friendly "not enrolled" explainer instead of an empty body.
