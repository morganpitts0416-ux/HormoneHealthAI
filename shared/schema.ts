...
export const patientCharts = pgTable("patient_charts", {
  ...
  // ── Smoking history (drives lung cancer screening / low-dose CT eligibility) ──
  // 'never' | 'former' | 'current' | null (unknown/not asked)
  smokingStatus: varchar("smoking_status", { length: 10 }),
  smokingPackYears: numeric("smoking_pack_years", { precision: 5, scale: 1 }),
  // Only meaningful when smokingStatus = 'former'
  smokingQuitDate: date("smoking_quit_date"),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});
...
export const clinicSpruceSettings = pgTable("clinic_spruce_settings", {
  ...
  // Master switch for June-assisted acknowledgments in Spruce conversations.
...
// ── spruce_workflow_settings ───────────────────────────────────────────────
// Per-clinic, per-workflow toggles for Spruce June Phase 3A.
// Controls whether June may send acknowledgments / follow-up questions for
// each workflow type, and how many AI turns are allowed per conversation.
// One row per (clinic_id, workflow) — created on first save; defaults apply
// when no row exists (acknowledgment OFF, follow-up OFF, maxTurns=1).
export const spruceWorkflowSettings = pgTable("spruce_workflow_settings", {
  id: serial("id").primaryKey(),
  clinicId: integer("clinic_id").notNull().references(() => clinics.id, { onDelete: "cascade" }),
  // Matches the workflow values in spruceWorkflowRequests.workflow
  workflow: varchar("workflow", { length: 50 }).notNull(),
  // Whether June may send an initial acknowledgment for this workflow type.
  // Default OFF — must be explicitly turned on per workflow.
  allowAcknowledgment: boolean("allow_acknowledgment").notNull().default(false),
  // Whether June may ask 1–2 safe follow-up questions after acknowledging.
  allowFollowUpQuestion: boolean("allow_follow_up_question").notNull().default(false),
  // Maximum number of June AI turns allowed per conversation for this workflow.
  maxJuneTurns: integer("max_june_turns").notNull().default(1),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
}, (t) => ({
  uniqClinicWorkflow: uniqueIndex("spruce_workflow_settings_clinic_workflow_idx").on(t.clinicId, t.workflow),
}));
export type SpruceWorkflowSettings = typeof spruceWorkflowSettings.$inferSelect;
export const insertSpruceWorkflowSettingsSchema = createInsertSchema(spruceWorkflowSettings).omit({ id: true, updatedAt: true });
export type InsertSpruceWorkflowSettings = z.infer<typeof insertSpruceWorkflowSettingsSchema>;
...
export const spruceWorkflowRequests = pgTable("spruce_workflow_requests", {
  ...
  // ── Spruce June Phase 3A fields ──────────────────────────────────────────
  // Set when June successfully sends an acknowledgment for this request.
  juneAckSentAt: timestamp("june_ack_sent_at"),
  // Structured staff-facing memo generated by June (updated each June turn).
  juneMemoText: text("june_memo_text"),
  // Number of June AI turns taken on this conversation (caps at maxJuneTurns).
  juneTurnCount: integer("june_turn_count").notNull().default(0),
  // Resolution tracking
...
export const clinicalOrders = pgTable("clinical_orders", {
  ...
  // Medication refill: auto-create next order after N weeks (e.g. testosterone every 10 weeks)
  recurrenceWeeks: integer("recurrence_weeks"),
  // Links every order generated by the same recurring chain. First order in
  // a chain leaves this null (its own id is treated as the series id);
  // subsequent auto-created orders point back to that original id.
  recurrenceSeriesId: integer("recurrence_series_id"),
  ...
  createdAt: timestamp("created_at").defaultNow().notNull(),
  // ── Medication refill order fields ──────────────────────────────────────
  medicationName: varchar("medication_name", { length: 150 }),
  medicationDose: varchar("medication_dose", { length: 100 }),
  pharmacyName: varchar("pharmacy_name", { length: 150 }),
  pharmacyPhone: varchar("pharmacy_phone", { length: 30 }),
  isControlledSubstance: boolean("is_controlled_substance").notNull().default(false),
  pmpChecked: boolean("pmp_checked").notNull().default(false),
  uaChecked: boolean("ua_checked").notNull().default(false),
  // Whether this refill requires the patient to be charged before dispensing
  requiresPayment: boolean("requires_payment").notNull().default(false),
  // Manually marked by staff once payment has been collected — no live Stripe charge
  ...
  seriesIdx: index("clinical_orders_series_idx").on(t.recurrenceSeriesId),
}));
...
// One row per (patient, screeningKey) — current status + latest completion
// snapshot. `screeningKey` matches an entry in server/screening-rules.ts
// (e.g. 'mammogram', 'pap_smear', 'dexa', 'psa', 'colonoscopy', 'lung_ct').
export const patientScreenings = pgTable("patient_screenings", {
  id: serial("id").primaryKey(),
  clinicId: integer("clinic_id").notNull().references(() => clinics.id, { onDelete: "cascade" }),
  patientId: integer("patient_id").notNull().references(() => patients.id, { onDelete: "cascade" }),
  screeningKey: varchar("screening_key", { length: 50 }).notNull(),
  // 'due' | 'overdue' | 'ordered' | 'completed'
  status: varchar("status", { length: 20 }).notNull().default("due"),
  // Whether this screening was auto-generated by the rules engine (age/gender/
  // risk-factor criteria) or manually added by staff for a patient outside
  // the normal guideline range. Manual entries are never removed by the
  // engine even if the patient falls outside current criteria.
  addedManually: boolean("added_manually").notNull().default(false),
  nextDueDate: date("next_due_date"),
  // ── Latest completion snapshot (mirrors the most recent patient_screening_events row) ──
  lastCompletedDate: date("last_completed_date"),
  lastOrderedBy: varchar("last_ordered_by", { length: 150 }),
  lastFacility: varchar("last_facility", { length: 200 }),
  lastResultSummary: text("last_result_summary"),
  lastLinkedDocumentId: integer("last_linked_document_id").references(() => patientDocuments.id, { onDelete: "set null" }),
  // Health-maintenance clinical_orders row sent for the current cycle, if any.
  linkedOrderId: integer("linked_order_id").references(() => clinicalOrders.id, { onDelete: "set null" }),
  // Staff closed the overdue flag without completing it. Re-flags automatically
  // 30 days later, or immediately if nextDueDate changes in the meantime.
  flagDismissedAt: timestamp("flag_dismissed_at"),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
}, (t) => ({
  patientScreeningIdx: uniqueIndex("patient_screenings_patient_clinic_key_idx").on(t.patientId, t.clinicId, t.screeningKey),
  clinicStatusIdx: index("patient_screenings_clinic_status_idx").on(t.clinicId, t.status),
}));
...
// Append-only history of completed screening events (mirrors medications-history
// pattern) so prior mammograms/pap smears etc. are never lost when a new one
// is recorded.
export const patientScreeningEvents = pgTable("patient_screening_events", {
  id: serial("id").primaryKey(),
  // clinic_id is nullable here only to allow safe migration of existing rows;
  // all new rows must supply it. The DB enforces NOT NULL once back-fill is done.
  clinicId: integer("clinic_id").references(() => clinics.id, { onDelete: "cascade" }),
  patientId: integer("patient_id").notNull().references(() => patients.id, { onDelete: "cascade" }),
  screeningKey: varchar("screening_key", { length: 50 }).notNull(),
  // 'completed' | 'ordered' | 'patient_reported'
  eventType: varchar("event_type", { length: 30 }).notNull(),
  eventDate: date("event_date").notNull(),
  orderedBy: varchar("ordered_by", { length: 150 }),
  facility: varchar("facility", { length: 200 }),
  resultSummary: text("result_summary"),
  linkedDocumentId: integer("linked_document_id").references(() => patientDocuments.id, { onDelete: "set null" }),
  linkedOrderId: integer("linked_order_id").references(() => clinicalOrders.id, { onDelete: "set null" }),
  // 'staff' (entered/verified by clinic staff) | 'patient_form' (came in via
  // an intake-form smart field, not yet clinically verified)
  source: varchar("source", { length: 20 }).notNull().default("staff"),
  // Nullable — user may be deleted without orphaning audit history
  recordedBy: integer("recorded_by").references(() => users.id, { onDelete: "set null" }),
  createdAt: timestamp("created_at").defaultNow().notNull(),
}, (t) => ({
  patientScreeningKeyIdx: index("patient_screening_events_patient_key_idx").on(t.patientId, t.screeningKey),
}));

export type PatientScreeningEvent = typeof patientScreeningEvents.$inferSelect;
export const insertPatientScreeningEventSchema = createInsertSchema(patientScreeningEvents).omit({
  id: true, createdAt: true,
});
export type InsertPatientScreeningEvent = z.infer<typeof insertPatientScreeningEventSchema>;
